module HistogramMsg
{
    ...
    proc histogram(a: [?aD] ?t, bins: int): [] int throws where (t==real) ||
                                                                (t==int) {
      ...
      if (bins <= sBound) {
        /*hgmLogger.debug(getModuleName(),getRoutineName(),getLineNumber(),*/
            /*"%? <= %?".format(bins,sBound));*/
        return histogramReduceIntent(a, aMin, aMax, bins, binWidth);
      }
      else if (bins <= mBound) {
        /*hgmLogger.debug(getModuleName(),getRoutineName(),getLineNumber(),*/
            /*"%? <= %?".format(bins,mBound));*/
        return histogramLocalAtomic(a, aMin, aMax, bins, binWidth);
      }
      else {
        /*hgmLogger.debug(getModuleName(),getRoutineName(),getLineNumber(),*/
            /*"%? > %?".format(bins,mBound));*/
        return histogramGlobalAtomic(a, aMin, aMax, bins, binWidth);
      }
    }
